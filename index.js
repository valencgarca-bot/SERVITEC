...
// 📂 BASE DE DATOS
const db = new sqlite3.Database('./servitec_ecuador_v1.db');

// 📧 NUEVAS CREDENCIALES DE CORREO
const MI_CORREO = 'nextra2024@gmail.com';
const MI_CLAVE = 'pwltnvksuudfqvkc'; 

app.use(express.urlencoded({ extended: true }));
app.use(session({
    secret: 'servitec_ultra_secure_2026_EC',
    resave: false,
...
// 🔥 ESTILOS PROFESIONALES Y MINIMALISTAS - INTERFAZ SERVITEC 🔥
const CSS_MODERNO = `
<style>
    ...
    :root {
        ...
        /* 🇪🇨 COLORES BANDERA DE ECUADOR */
        --ec-yellow: #ffdd00;
        --ec-yellow-dim: rgba(255, 221, 0, 0.1);
        --ec-blue: #007bff;
        --ec-blue-dim: rgba(0, 123, 255, 0.1);
        --ec-red: #ff1744;
        --ec-red-dim: rgba(255, 23, 68, 0.1);
    ...
    body::after { content: ""; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 15% 15%, rgba(255, 221, 0, 0.05) 0%, transparent 40%), radial-gradient(circle at 85% 85%, rgba(255, 23, 68, 0.05) 0%, transparent 40%); z-index: -1; }
    ...
    .top-header h2 { margin: 0; font-size: 20px; font-weight: 800; color: var(--text-primary); display: flex; align-items: center; gap: 10px; }
    ...
    input:focus, select:focus, textarea:focus { border-color: var(--ec-yellow); outline: none; background: rgba(0,0,0,0.6); box-shadow: 0 0 12px rgba(255,221,0,0.1); }
    ...
    .btn-yellow-ec { background: linear-gradient(135deg, #ffdd00 0%, #d4b800 100%); color: #000 !important; border-color: #ffdd00; box-shadow: 0 6px 20px rgba(255,221,0,0.18); }
    .btn-blue-ec { background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: #fff !important; font-weight: 800 !important; border-color: #007bff; box-shadow: 0 6px 20px rgba(0,123,255,0.18); }
    .btn-red-ec { background: linear-gradient(135deg, #ff1744 0%, #a00 100%); color: #fff !important; border-color: #ff1744; box-shadow: 0 6px 20px rgba(255,0,0,0.18); }
    ...
    .danger-btn-sidebar:hover { background: rgba(255, 23, 68, 0.08); border-color: var(--ec-red); color: var(--ec-red); }
...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        .logo-ec { font-size: 40px; font-weight: 800; margin-bottom: 12px; text-transform: uppercase; letter-spacing: -1.2px; line-height: 1.1; }
        .logo-ec .yellow { color: var(--ec-yellow); } 
        .logo-ec .blue { color: var(--ec-blue); } 
        ...
        input:focus { border-color: var(--ec-yellow); outline: none; background: rgba(0,0,0,0.8); box-shadow: 0 0 12px rgba(255,221,0,0.2); }
        
        .btn-neon-yellow { background: linear-gradient(135deg, #ffdd00 0%, #d4b800 100%); color: #000; border: none; padding: 18px; border-radius: 10px; cursor: pointer; font-weight: 800; width: 100%; text-transform: uppercase; font-size: 15px; letter-spacing: 0.8px; border: 1px solid var(--ec-yellow); box-shadow: 0 6px 25px rgba(255,221,0,0.25); transition: all 0.2s ease; font-family: 'Inter', sans-serif; }
        .btn-neon-yellow:hover { opacity: 0.9; transform: translateY(-1.5px); box-shadow: 0 8px 30px rgba(255,221,0,0.35); }
        ...
        .lang-selector-login select:hover { box-shadow: 0 4px 20px rgba(255,221,0,0.2); }
    </style>
    `;
    ...
    <div class="login-panel">
        <div class="logo-ec">
            <span class="yellow">⚡ SER</span><span class="blue">VI</span><span class="red">TEC</span>
        </div>
...
// Ruta simple para cerrar sesión manualmente (añadida para completar el ciclo)
...
app.get('/dash', (req, res) => {
    ...
    if (esAdminPrincipal || esSubAdmin) {
        ...
        // VISTA PARA ROL "CLIENTE" (Cierre de tu código original)
        res.send(`
        ${CSS_MODERNO}
        <div style="position: absolute; width: 0; height: 0; overflow: hidden; z-index: -1;">
            <div id="google_translate_element"></div>
        ...
        <div class="main-content" style="text-align:center; padding-top: 80px;">
            <h3 style="color: var(--ec-yellow); font-size: 24px;">¡Bienvenido a SERVITEC!</h3>
            <p style="color: var(--text-secondary); max-width: 500px; margin: 20px auto; line-height: 1.6;">
                Actualmente tu cuenta tiene nivel de Cliente. Por favor, comunícate con tu administrador o subadministrador para la gestión de tus correos y contraseñas.
            </p>
        </div>
        </body>`);
    }
...
app.listen(PORT, () => {
    console.log(\`🚀 Servidor SERVITEC ejecutándose en http://localhost:${PORT}\`);
});
