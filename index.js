...
app.use(session({
    secret: 'servitec_ecuador_ultra_secure_2026',
    resave: false,
...
<style>
    ...
    :root {
        ...
        --ec-yellow: #ffce00;
        --ec-yellow-dim: rgba(255, 206, 0, 0.1);
        --ec-red: #ed1c24;
        --ec-red-dim: rgba(237, 28, 36, 0.1);
        --ec-blue: #004ea1;
    ...
    @keyframes led-glow { 
        ...
        33% { border-color: var(--ec-blue); box-shadow: 0 0 10px rgba(0,78,161,0.1); } 
    ...
    body::after { content: ""; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: radial-gradient(circle at 15% 15%, rgba(255, 206, 0, 0.05) 0%, transparent 40%), radial-gradient(circle at 85% 85%, rgba(237, 28, 36, 0.05) 0%, transparent 40%); z-index: -1; }
    ...
    input:focus, select:focus, textarea:focus { border-color: var(--ec-yellow); outline: none; background: rgba(0,0,0,0.6); box-shadow: 0 0 12px rgba(255,206,0,0.1); }
    ...
    .btn-yellow-ec { background: linear-gradient(135deg, #ffce00 0%, #d4aa00 100%); color: #000 !important; border-color: #ffce00; box-shadow: 0 6px 20px rgba(255,206,0,0.18); }
    .btn-blue-ec { background: linear-gradient(135deg, #004ea1 0%, #002366 100%); color: #fff !important; font-weight: 800 !important; border-color: #004ea1; box-shadow: 0 6px 20px rgba(0,78,161,0.18); }
    .btn-red-ec { background: linear-gradient(135deg, #ed1c24 0%, #a00 100%); color: #fff !important; border-color: #ed1c24; box-shadow: 0 6px 20px rgba(237,28,36,0.18); }
    ...
    .danger-btn-sidebar:hover { background: rgba(237, 28, 36, 0.08); border-color: var(--ec-red); color: var(--ec-red); }
...
app.get('/', (req, res) => {
    ...
    <style>
        ...
        .login-panel { position: relative; background: var(--bg-panel); padding: 70px 50px; border-radius: 24px; border: 1px solid rgba(255, 255, 255, 0.05); max-width: 440px; width: 90%;
        text-align: center; box-shadow: 0 25px 70px rgba(0,0,0,0.8); box-sizing: border-box; }
        
        .logo-ec { font-size: 32px;
        font-weight: 800; margin-bottom: 12px; text-transform: uppercase; letter-spacing: -1.2px; line-height: 1.1;
        }
        .logo-ec .yellow { color: var(--ec-yellow); } .logo-ec .blue { color: var(--ec-blue);
        } .logo-ec .red { color: var(--ec-red); }
        
        h2 { color: #b0b0b0;
        margin-bottom: 40px; font-weight: 500; font-size: 15px; margin-top: 0; line-height: 1.5;
        }
        
        input { width: 100%;
        padding: 18px; margin-bottom: 18px; border-radius: 10px; border: 1px solid #333; background: rgba(0,0,0,0.6); color: white; box-sizing: border-box; font-size: 16px; font-weight: 500;
        transition: 0.2s; }
        input:focus { border-color: var(--ec-yellow); outline: none; background: rgba(0,0,0,0.8);
        box-shadow: 0 0 12px rgba(255,206,0,0.2); }
        
        .btn-neon-yellow { background: linear-gradient(135deg, #ffce00 0%, #d4aa00 100%);
        color: #000; border: none; padding: 18px; border-radius: 10px; cursor: pointer; font-weight: 700; width: 100%; text-transform: uppercase; font-size: 15px; letter-spacing: 0.8px;
        border: 1px solid var(--ec-yellow); box-shadow: 0 6px 25px rgba(255,206,0,0.25); transition: all 0.2s ease; font-family: 'Inter', sans-serif;
        }
        .btn-neon-yellow:hover { opacity: 0.9; transform: translateY(-1.5px); box-shadow: 0 8px 30px rgba(255,206,0,0.35);
        }
        ...
        .lang-selector-login { position: absolute;
        top: 30px; right: 40px; z-index: 1000; }
        .lang-selector-login select { background: rgba(18,18,18,0.9);
        color: white; border: 1px solid var(--ec-yellow); padding: 10px 15px; border-radius: 10px; font-family: 'Inter', sans-serif; font-weight: 600; font-size: 14px; cursor: pointer;
        outline: none; box-shadow: 0 4px 15px rgba(0,0,0,0.5); transition: 0.2s; }
        .lang-selector-login select:hover { box-shadow: 0 4px 20px rgba(255,206,0,0.2);
        }
    </style>
    `;
    ...
    <div class="login-panel">
        <div class="logo-ec"><span class="yellow">⚡ SER</span><span class="blue">VI</span><span class="red">TEC</span></div>
    ...
    if (esAdminPrincipal || esSubAdmin) {
        ...
        db.all(query, params, (err, usuarios) => {
            ...
            db.all("SELECT * FROM correos", [], (err, correos) => {
                ...
                if (esSubAdmin) subadmins = usuarios.filter(u => 
u.id === req.session.uid);
                ...
                subadmins.forEach(sub => {
                    ...
                    <details class="folder item-folder">
                        ...
                        <div class="folder-content">
                            <a href="/admin/del-user/${sub.id}" class="action-btn-link btn-red-ec" style="margin-bottom:25px; padding: 10px; font-size:11px;"
                            onclick="return confirm('⚠️ ¿Borrar a este subadmin y sus clientes?')">❌ ELIMINAR SUBADMINISTRADOR</a>
                            ...
                            clientesDelSub.forEach(cli => {
                                ...
                                <div class="client-card item-client" style="border-left-color: var(--ec-yellow);">
                                    ...
                                    <form action="/admin/add-mail-masivo" method="POST" style="margin-top:15px; display:flex; gap:8px;">
                                        ...
                                        <button class="action-btn btn-yellow-ec" style="width:auto;
                                        padding:10px 15px; font-size:12px;">📥</button>
                                    </form>
                ...
                if (esAdminPrincipal) {
                    ...
                    if(clientesDirectos.length > 0) {
                        ...
                        margin-top:40px; border-bottom:1px solid var(--border-color); padding-bottom:12px; font-size:16px;">👤 Clientes Directos</h4>
                        ...
                        clientesDirectos.forEach(cli => {
                            ...
                            clientesDirectosHtml += `
                                ...
                                <div class="client-card item-client" style="border-left-color: var(--ec-red);">
                                <strong style="color: var(--ec-red);">👤 ${cli.user}</strong>
                                <a href="/admin/del-user/${cli.id}" class="del-btn" onclick="return confirm('¿Borrar 
                                cliente?')">×</a>
                                <form action="/admin/add-mail-masivo" method="POST" style="margin-top:15px; display:flex; gap:8px;">
                                    <input type="hidden" name="uid" value="${cli.id}">
                        
                                    <input name="emails" placeholder="Pega los correos..." style="padding:10px; margin:0; font-size:13px;">
                                    <button class="action-btn btn-red-ec" style="width:auto;
                                    padding:10px 15px; font-size:12px;">📥</button>
                                </form>
                                <div class="email-list">`;
                ...
                <div class="top-header">
                    <h2><span class="brand-ec">⚡ SERVITEC</span></h2>
                    <div style="display:flex; align-items:center; gap:20px;">
                        ...
                        <span class="user-badge">${req.session.user} |
                        ${req.session.rol.toUpperCase()}</span>
                ...
                <div class="dashboard-layout">
                    ...
                    <div class="sidebar">
                        ...
                        <button class="tab-btn" onclick="openTab('panel-buscar')">🔎 Buscar 
                        Dueño de Cuenta</button>
                    ...
                    <div class="main-content">
                        <div id="panel-stream" class="tab-panel">
                            ...
                            <form action="/buscar" method="POST">
                                <input name="email_search" placeholder="Escribe el correo a buscar (Netflix, Disney, etc)..." required style="border-color: #444;
                                background: rgba(255,255,255,0.03);">
                                <div style="display: grid;
                                grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 12px; margin-top: 10px;">
                        ...
                        <div id="panel-buscar" class="tab-panel">
                            ...
                            <input type="text" id="buscadorLocal" onkeyup="buscarCorreoLocal();
                            openTab('panel-usuarios');" placeholder="Escribe un correo o nombre aquí para filtrar..." style="border-color:var(--ec-yellow);">
                        </div>
                        ...
                        <div id="panel-usuarios" class="tab-panel">
                            ...
                            ${subadminsHtml}
                            ${clientesDirectosHtml}
                            ${(!subadminsHtml && !clientesDirectosHtml) ?
                            '<p style="color:#555; text-align:center;">No hay usuarios registrados aún en el sistema.</p>' : ''}
                        </div>
                    </div>
                </div>
                </body>`);
        ...
        <div style="padding: 60px 20px; display:flex; justify-content:center;">
            <div class="tab-panel active" style="margin: 0;
            width:100%; max-width:600px; display: block; padding: 40px;">
                ...
                <form action="/buscar" method="POST">
                    ...
                    border-color: #444; background: rgba(255,255,255,0.03);">
                    <div style="display: grid;
...
app.post('/buscar', async (req, res) => {
    ...
    try {
        ...
        if (accion === 'pais') {
            ...
            const reglasPais = [
                { id: "🇺🇸 Estados Unidos", keys: ['ee.
                uu.', 'usa', 'united states', 'los gatos', 'california', '1-866-', '1-844-', '1-800-', '1-888-', '1-877-'] },
                ...
                { id: "🇳🇱 Sede Europea (Holanda)", keys: ['netherlands', 
                'holanda', 'países bajos', 'amsterdam', 'international b.v.', 'b.v.'] },
                ...
                { id: 
                "🇵🇹 Portugal", keys: ['portugal', 'lisboa', 'lisbon'] },
            ...
            `<div style="font-size: 50px; margin: 40px auto; padding: 30px; background:#fff; color:#000; border-radius:15px; display:inline-block; border-bottom: 5px solid #ed1c24; border-top: 5px solid #ffce00;">${paisDetectado}</div>` : `<div style="margin: 30px auto; padding: 20px; background:#222; border-radius:15px; display:inline-block; border: 1px solid #ffaa00;"><h3 style="color:#ffaa00; margin-top:0;">⚠️ País no detectado</h3></div>`;
        ...
        if (accion === 'ip') {
            ...
            return res.send(`<div style="background:#000; text-align:center; padding:15px;"><a href="/dash" style="color:#fff; text-decoration:none; border: 1px solid #fff; padding: 8px 15px; border-radius: 5px; font-family:'Inter', sans-serif;">⬅ VOLVER AL PANEL</a></div><div style="background:#111; color:white; padding: 40px; text-align:center; font-family:'Inter', sans-serif; min-height:100vh;"><h2>📡 Registro de IP</h2><p>Correo: <strong style="color:var(--ec-yellow);">${email_search}</strong></p><div style="margin: 40px auto; padding: 30px; background:#222; border-radius:15px; display:inline-block; border: 1px solid #f00;">${ipContenido}</div></div>`);
        }
